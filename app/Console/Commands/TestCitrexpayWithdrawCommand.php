...
class TestCitrexpayWithdrawCommand extends Command
{
    use CitrexpayApi; // Usa o seu Trait

    protected $signature = 'test:gateway:citrex-withdraw';
    protected $description = 'Testa a integração com um gateway de pagamento para método se saque, de forma isolada';
    ...
    public function handle()
    {
        ...
        $withdrawInfo = new stdClass();
        $withdrawInfo->user = $user;
        $withdrawInfo->document = '36397294803';
        $withdrawInfo->amount = 1;
        $withdrawInfo->amount_tax = false;
        $withdrawInfo->transactionId = null;

        $this->info("Valor do depósito de teste: R$ {$withdrawInfo->amount}");
        ...
        $this->info('Chamando o método withdrawCitrexpay...');
        try {
            $response = $this->withdrawCitrexpay($withdrawInfo);

            // 5. Exibir o resultado
            $this->info('Resposta do método:');
            print_r($response->getData());

            if (isset($response->getData()->status) && $response->getData()->status == 1) {
